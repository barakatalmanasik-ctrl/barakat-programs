      ...
      <tr>
        ...
        <td>
          <div class="admin-table__actions">
            ...
            <button class="admin-btn--icon" title="حذف الصورة" onclick="deleteAdminGalleryImage('${img.id}')" style="color:var(--color-error)">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
            </button>
          </div>
        </td>
...
async function deleteAdminGalleryImage(id) {
  ...
  try {
    const { error } = await SupabaseClient
      ...
      .eq('id', id);
    if (error) throw error;
    showAdminToast('تم حذف الصورة بنجاح', 'success');
    await loadAdminGallery();
  } catch (e) {
    showAdminToast(e.message || 'تعذر حذف الصورة', 'error');
  }
}
